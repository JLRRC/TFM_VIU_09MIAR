#!/usr/bin/env bash
# run_directo2_validation.sh — Orquestador de corrida fresca de DIRECTO2.
# Lanza: panel (con runner offscreen) + capture + benchmark y recoge resultados.
# Análogo a run_directo_validation.sh pero para el pipeline DIRECTO2.
#
# Uso:
#   ./scripts/run_directo2_validation.sh
#
# Variables de entorno opcionales:
#   DIRECTO2_TIMEOUT_SEC  — timeout total del runner (default: 300s)
...
RUN_ID="directo2_validation_$(date +%Y%m%d_%H%M%S)"
...
echo "[ORCH] Corrida DIRECTO2: $RUN_ID"
...
pkill -f "run_directo2_button_offscreen" 2>/dev/null || true
pkill -f "run_directo_button_offscreen" 2>/dev/null || true
sleep 4
# Limpiar shared memory residual de FastDDS/CycloneDDS
rm -f /dev/shm/fastrtps_* /dev/shm/sem.fastrtps_* 2>/dev/null || true
echo "[ORCH] Limpieza completada"

# --- Lanzar el runner del panel offscreen para DIRECTO2 ---
env \
    ...
    DIRECTO2_CLICK_DELAY_MS=8000 \
    "DIRECTO2_EXIT_AFTER_MS=$(( DIRECTO2_TIMEOUT_SEC * 1000 ))" \
    DIRECTO2_RETRY_MS=5000 \
    DIRECTO2_MAX_ATTEMPTS=90 \
    python3 "$SCRIPT_DIR/run_directo2_button_offscreen.py" \
    >"$HELPER_LOG" 2>&1 &
HELPER_PID=$!
...
python3 "$SCRIPT_DIR/grasp_audit_benchmark.py" \
    --run "DIRECTO2:directo2:$TRACE_FILE" \
