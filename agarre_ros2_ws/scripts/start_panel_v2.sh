...
# Preflight a PICK_IMAGE para que MoveIt planifique desde una pose cercana al objeto.
# Evita que OMPL elija IK con shoulder_pan alejado del plano de agarre (y=0).
# Modo "home_pick_image": HOME_START primero, luego move a PICK_IMAGE antes de APPROACH.
export PANEL_PICK_OBJECT_PREFLIGHT_MODE="${PANEL_PICK_OBJECT_PREFLIGHT_MODE:-home_pick_image}"
...
# HOME_START garantiza que el robot parta de HOME antes de planificar con MoveIt.
# Evita fallos del controlador al planificar desde configuraciones alejadas de HOME.
...
export PANEL_MOVEIT_BRIDGE_PATH_CONSTRAINT_TOL_RAD="${PANEL_MOVEIT_BRIDGE_PATH_CONSTRAINT_TOL_RAD:-1.5}"
