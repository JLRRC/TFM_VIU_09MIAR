...
"""Helpers del /system_state externo para el panel."""
...
EXTERNAL_STATE_LIVENESS_SEC = 2.0


def _worker_external_state_snapshot(panel) -> Optional[Tuple[str, str, float]]:
    worker = getattr(panel, "ros_worker", None)
    if worker is None:
        return None
    getter = getattr(worker, "system_state_snapshot", None)
    if not callable(getter):
        return None
    try:
        state, reason, wall = getter()
    except Exception:
        return None
    ...
    if wall_f <= 0.0:
        return None
    return state, str(reason or ""), wall_f


def _fresh_external_state_from_worker(panel) -> Optional[Tuple[str, str]]:
    snapshot = _worker_external_state_snapshot(panel)
    if snapshot is None:
        return None
    state, reason, wall = snapshot
    if (time.monotonic() - wall) >= EXTERNAL_STATE_LIVENESS_SEC:
        return None
    return state, reason


def external_state_active(panel) -> bool:
    if panel._external_state and (time.time() - panel._external_state_last) < EXTERNAL_STATE_LIVENESS_SEC:
        return True
    return _fresh_external_state_from_worker(panel) is not None


def resolve_external_state(panel) -> Tuple[Optional[SystemState], str]:
    worker_state = _fresh_external_state_from_worker(panel)
    if worker_state is not None:
        state, worker_reason = worker_state
        reason = worker_reason or panel._external_state_reason or ""
    else:
