...
def _step_window_refresh_history_table(panel, *, operational_live) -> None:
    """F3-step19a: refresca la tabla _step_history_table con todas las filas
    actuales (~78 LOC). Cada fila muestra Org/TCP-TF/Obj/Target/Exec en world
    + distancias D TCP-Obj/D Target-Obj/Err TCP-Exec + Tipo Target + Razón +
    Estado. Usa compute_step_history_metrics para los deltas.
    """
    if panel._step_history_table is None:
        return
...
def _step_window_refresh_mesh_align(panel, *, operational_frame, operational_live, operational_live_display) -> None:
    """F3-step19b: emite el log [MESH_ALIGN] comparando frames tool0/pinch/
    obj_root/step_org en el mismo ciclo de refresco. Útil para detectar
    desfases entre UI y geometría runtime.
    """
    try:
        _ma_tool0_base = panel._step_fetch_live_pose("tool0")
        _ma_pinch_base = operational_live
        _ma_obj_world = panel._step_fetch_object_world()
        _ma_tool0_world = panel._step_display_position(_ma_tool0_base)
        _ma_pinch_world = operational_live_display
...
def _step_window_refresh(panel) -> None:
    ...
    _step_window_refresh_mesh_align(
        panel,
        operational_frame=operational_frame,
        operational_live=operational_live,
        operational_live_display=operational_live_display,
    )
    ...
    _step_window_refresh_history_table(panel, operational_live=operational_live)
